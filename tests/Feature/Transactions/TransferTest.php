...
final class TransferTest extends TransactionTestCase
{
    ...
    public function authenticatedUserCanExecuteAMonoTransferWithToken(): void
    {
        ...
        self::assertSame('Transfer executed successfully', $body['message']);
    ...
    public function authenticatedUserCanInitializeAMultiTransfer(): void
    {
        ...
        $this->client->jsonRequest(
            'POST', '/api/transactions/init-multi-transfer',
            ['transfers' => [$this->item($one, 30, 'First payment'),
            $this->item($two, 20, 'Second payment')]]
        );
        self::assertResponseStatusCodeSame(201);
        $body = $this->jsonResponse();
        self::assertArrayHasKey('status_code', $body);
        self::assertCount(2, $body['data']);
        $this->assertTransfer($one, '30.000000', StatusTransfer::PENDING, 'First payment');
        $this->assertTransfer($two, '20.000000', StatusTransfer::PENDING, 'Second payment');
    }

    #[Test]
    public function authenticatedUserCanExecuteAMultiTransferWithTokens(): void
    {
        $one = $this->account('ACC-111-111');
        $two = $this->account('ACC-222-222');
        $this->client->jsonRequest(
            'POST', '/api/transactions/init-multi-transfer',
            ['transfers' => [$this->item($one, 30, 'First payment'),
            $this->item($two, 20, 'Second payment')]]
        ...
        $this->execute(
            '/api/transactions/execute-multi-transfer',
            ['tokens' => $tokens]);
            self::assertResponseStatusCodeSame(201);
        $body = $this->jsonResponse();
        self::assertSame(201, $body['status_code']);
        self::assertSame('Multi transfer executed successfully', $body['message']); 
        self::assertCount(2, $body['data']);
        self::assertSame('completed', $body['data'][0]['status']);
        self::assertSame('completed', $body['data'][1]['status']);
        $this->assertTransferToken($tokens[0], '30.000000'); 
        $this->assertTransferToken($tokens[1], '20.000000'); 
        $this->assertBalances('95.000000', '30.000000', '5.150000');
    }
    ...
    public function executeMultiTransferWithInvalidTokensReturnsError(): void
    {
        ...
        $this->assertValidationError('tokens[0]'); 
        $this->assertValidationError('tokens[1]');
    }
    ...
    public function senderAccountMustExist(): void
    {
        ...
        $this->assertValidationError('from_account_number');
    }
    ...
    public function receiverAccountMustExist(): void
    {
        ...
        self::assertResponseStatusCodeSame(422); 
        $this->assertValidationError('to_account_number');
    }
    ...
    public function sameAccountTransferChargesFee(): void
    {
        ...
        $account = $this->entityManager->find(Account::class, $this->account->getId());
 ...
 { 
    ...
    private function execute(string $path, array $payload): void 
    { 
        $this->client->jsonRequest('POST', $path, $payload); 
    }
    
    private function assertTransfer(Account $receiver, string $amount, StatusTransfer $status, ?string $description = null): void 
    { 
        $query = $this->entityManager->getRepository(Transfer::class)->findOneBy(['senderAccount' => $this->account, 'receiverAccount' => $receiver, 'currency' => $this->currency, 'type' => TypeTransfer::TRANSFER, 'status' => $status]); 
        self::assertNotNull($query);
        self::assertSame($amount, number_format((float) $query->getAmount(), 6, '.', ''));
        if ($description !== null) self::assertSame($description, $query->getDescription()); 
    }
    
    private function assertTransferToken(string $token, string $amount): void 
    { 
        $transfer = $this->entityManager->getRepository(Transfer::class)->findOneBy(['token' => $token]);
        self::assertNotNull($transfer);
        self::assertSame($amount, number_format((float) $transfer->getAmount(), 6, '.', ''));
        self::assertSame(StatusTransfer::COMPLETED, $transfer->getStatus());
    }
    
    private function assertBalances(string $sender, string $receiver, string $system): void 
    {
         $this->entityManager->clear(); 
         $account = $this->entityManager->find(Account::class, $this->account->getId());
         $number = $receiver === '50.000000' ? 'ACC-654-321' : ($receiver === '30.000000' ? 'ACC-111-111' : 'ACC-222-222');
         $receiverAccount = $this->entityManager->getRepository(Account::class)->findOneBy(['accountNumber' => $number]);
         $systemAccount = $this->entityManager->find(Account::class, $this->systemAccount->getId());
         self::assertSame(number_format((float) $sender, 2, '.', ''), number_format((float) $account?->getBalance(), 2, '.', ''));
         self::assertSame(number_format((float) $receiver, 2, '.', ''), number_format((float) $receiverAccount?->getBalance(), 2, '.', ''));
         self::assertSame(number_format((float) $system, 2, '.', ''), number_format((float) $systemAccount?->getBalance(), 2, '.', ''));
  }
    
  private function assertValidationError(string $field): void 
  {
     $body = $this->jsonResponse();
     self::assertTrue(array_reduce($body['violations'] ?? [], static fn(bool $found, array $v): bool => $found || ($v['propertyPath'] ?? null) === $field, false), 'Expected validation error for '.$field); 
  }
